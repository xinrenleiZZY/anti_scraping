...
window.openBatchDeleteModalOv = async function() {
    await loadKeywords();
    const tagSet = new Set();
    for (const kw of Object.keys(kwTagsMap)) {
        (kwTagsMap[kw] || []).forEach(t => tagSet.add(t));
    }
    const sel = document.getElementById('bdOvFilterTag');
    sel.innerHTML = '<option value="">全部标签</option>' + Array.from(tagSet).sort().map(t => `<option value="${t}">${t}</option>`).join('');
    renderBatchDeleteListOv();
    new bootstrap.Modal(document.getElementById('batchDeleteModalOv')).show();
};

window.renderBatchDeleteListOv = function() {
    const filterTag = document.getElementById('bdOvFilterTag').value;
    const keywords = Object.keys(kwTagsMap).filter(kw => {
        if (filterTag && !(kwTagsMap[kw] || []).includes(filterTag)) return false;
        return true;
    });
    document.getElementById('bdOvKeywordCount').textContent = `共 ${keywords.length} 个关键词`;
    document.getElementById('bdOvKeywordList').innerHTML = keywords.length
        ? keywords.map(kw => `<label class="d-flex align-items-center py-1 px-2 border-bottom"><input type="checkbox" class="bd-ov-kw-checkbox me-2" value="${kw.replace(/"/g, '&quot;')}">${kw}</label>`).join('')
        : '<div class="text-muted text-center py-3">无匹配关键词</div>';
};

window.bdOvToggleAll = function(source) {
    document.querySelectorAll('.bd-ov-kw-checkbox').forEach(cb => cb.checked = source.checked);
};

window.confirmBatchDeleteOv = async function() {
    const checked = document.querySelectorAll('.bd-ov-kw-checkbox:checked');
    ...
    bootstrap.Modal.getInstance(document.getElementById('batchDeleteModalOv')).hide();
    loadKeywords();
};
