...
function renderTableRow(item) {
    const cells = visibleColumns.map(key => {
        ...
        // ========== 0425 新增字段格式化 ==========
            ...
            if (value === '大节日') {
                return `<td><span class="tag tag-red">${value}</span></td>`;
            } else if (value === '小节日') {
                return `<td><span class="tag tag-orange">${value}</span></td>`;
            }
            return `<td>${value || '-'}</td>`;
        }
            ...
            return `<td><span class="tag tag-gray">${value}</span></td>`;
        }
        else if (key === 'tags' && value) {
            if (Array.isArray(value)) {
                const badges = value.map(t => `<span class="tag tag-blue">${escapeHtml(t)}</span>`).join('');
                return `<td>${badges || '-'}</td>`;
            }
            return `<td>-</td>`;
        }
        else if (key === 'owner' && value) {
            return `<td><span class="tag tag-purple">${escapeHtml(value)}</span></td>`;
        }
        // ==================================
        else if (key === 'is_prime' && value === true) {
            return `<td><i class="bi bi-check-lg text-primary"></i> Prime</td>`;
        }
        // 普通文本 0425新增空值处理
...
window.searchData = async function(page = 1) {
    ...
    try {
        ...
        document.getElementById('dataTableBody').innerHTML = `｜DSML｜<td colspan="13" class="text-center text-danger">加载失败</td></tr>`;
    }
    try {
        const result = await apiFetch(`/results?${params}`);
        const tbody = document.getElementById('dataTableBody');
...
window.resetFilters = function() {
    ...
    // 取消所有关键词复选框
    document.querySelectorAll('#keywordOptions input[type="checkbox"]').forEach(cb => {
        cb.checked = false;
    });
    ...
    selectedTags = [];
    updateTagButtonText();
    document.querySelectorAll('#tagOptions input[type="checkbox"]').forEach(cb => {
        cb.checked = false;
    });
    ...
    document.getElementById('filterFestival').value = '';
    document.getElementById('filterFestivalType').value = '';
    document.getElementById('filterHotSeason').value = '';
...
async function loadFilterOptions() {
    ...
    allKeywords = Array.isArray(keywordsRes) ? keywordsRes : (keywordsRes.keywords || []);
    
    // 2. 加载标签选项（去重）
    const tagsSet = new Set();
    for (const kw of allKeywords) {
        const tags = await apiFetch(`/keywords/${encodeURIComponent(kw)}/tags`);
        tags.forEach(t => tagsSet.add(t));
    }
    allTags = Array.from(tagsSet).sort();
    
    // 3. 加载节日选项
    // 这里简单起见，先写死常用节日，或者从后端接口获取
    allFestivals = ['春节', '中秋节', '圣诞节', '夏季', '冬季'];
...
function renderMultiSelectOptions() {
    ...
    if (keywordContainer) {
        keywordContainer.innerHTML = allKeywords.map(kw => `
            <label class="multi-select-option">
                <input type="checkbox" value="${kw}" onchange="toggleKeywordSelection('${kw}', this.checked)">
                <span>${kw}</span>
            </label>
        `).join('');
    }
    ...
    if (tagContainer) {
        tagContainer.innerHTML = allTags.map(tag => `
            <label class="multi-select-option">
                <input type="checkbox" value="${tag}" onchange="toggleTagSelection('${tag}', this.checked)">
                <span>${tag}</span>
            </label>
        `).join('');
    }
    ...
    if (festivalSelect) {
        festivalSelect.innerHTML = '<option value="">全部节日</option>' + 
            allFestivals.map(f => `<option value="${f}">${f}</option>`).join('');
    }
}

// 记录选中的关键词
function toggleKeywordSelection(keyword, isChecked) {
    if (isChecked) {
    ...
    asinInput.addEventListener('input', function() {
        ...
        if (filtered.length) {
            asinDropdown.innerHTML = filtered.map(asin => 
                `<div class="autocomplete-item" onclick="selectAsin('${asin}')">${asin}</div>`
            ).join('');
...
document.addEventListener('DOMContentLoaded',async () => {
    ...
    // await loadFilterOptions();     // 加载标签、节日等选项
    // await initAsinAutocomplete();  // 初始化ASIN联想
    // if (!e.target.closest('#keywordMultiSelect')) {
    //     document.getElementById('keywordDropdown')?.classList.remove('show');
    // }
    // if (!e.target.closest('#tagMultiSelect')) {
